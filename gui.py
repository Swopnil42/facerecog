...
def save():
    try:
        con=pymysql.connect(user='root', password='lopuashu', host='localhost', database='root')
        cur=con.cursor()
        sql="inster into student values('%s','%s','%s')"\
...
b2=Button(w1, text=' DisplayAll ', command=displayall)
